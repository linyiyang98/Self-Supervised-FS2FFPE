...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'ranse+chaofen':
        net = ResnetMultiGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
    else:
...
class ResnetMultiGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(ResnetMultiGenerator, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        for i in range(2):  # add ResNet blocks
            model_ranse += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                  use_bias=use_bias)]
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            if no_antialias_up:
                model_ranse += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                             kernel_size=3, stride=2,
                                             padding=1, output_padding=1,
                                             bias=use_bias),
                          norm_layer(int(ngf * mult / 2)),
                ...
                model_ranse += [Upsample(ngf * mult),
                          ...
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True)]
        model_ranse += [nn.ReflectionPad2d(3)]
        model_ranse += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model_ranse += [nn.Tanh()]
        self.model_ranse = nn.Sequential(*(model+model_ranse))

        model_chaofen = []
        mult = 2 ** n_downsampling
        for i in range(2):  # add ResNet blocks
            model_chaofen += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                  use_bias=use_bias)]
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            if no_antialias_up:
                model_chaofen += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                             kernel_size=3, stride=2,
                                             padding=1, output_padding=1,
                                             bias=use_bias),
                          norm_layer(int(ngf * mult / 2)),
                ...
                model_chaofen += [Upsample(ngf * mult),
                          nn.Conv2d(ngf * mult, int(ngf * mult / 2),
                                    kernel_size=3, stride=1,
                                    padding=1,  # output_padding=1,
                                    bias=use_bias),
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True)]
        model_chaofen += [nn.ReflectionPad2d(3)]
        model_chaofen += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model_chaofen += [nn.Tanh()]
        self.model_chaofen = nn.Sequential(*(model+model_chaofen))

    def forward(self, input, layers=[], encode_only=False):
        if -1 in layers:
            layers.append(len(self.model_ranse))
        if len(layers) > 0:

            feat = input
            feats = []
            for layer_id, layer in enumerate(self.model_ranse):
                # print(layer_id, layer)
                feat = layer(feat)
            ...
            """Standard forward"""
            fake = self.model_ranse(input)
            return fake

    def forward_chaofen(self, input):
        return self.model_chaofen(input)
