...
class CUTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_D_low = torch.optim.Adam(self.netD_low.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizers.append(self.optimizer_G)
            self.optimizers.append(self.optimizer_D_high)
            self.optimizers.append(self.optimizer_D_low)
    ...
    def optimize_parameters(self):
        ...
        self.set_requires_grad(self.netD_low, True)
        self.optimizer_D_low.zero_grad()
        self.loss_D_low = self.compute_D_high_loss()
        self.loss_D_low.backward()
        self.optimizer_D_low.step()

        # update G
        self.set_requires_grad(self.netD_high, False)
        self.set_requires_grad(self.netD_low, False)
    ...
    def compute_D_high_loss(self):
        ...
        self.loss_D_high = (self.loss_D_fake + self.loss_D_real) * 0.5
    ...
    def compute_D_low_loss(self):
        ...
        self.loss_D_fake = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netD_low(self.real_B)
        loss_D_real = self.criterionGAN(self.pred_real, True)
        self.loss_D_real = loss_D_real.mean()

        # combine loss and calculate gradients
        self.loss_D_low = (self.loss_D_fake + self.loss_D_real) * 0.5
