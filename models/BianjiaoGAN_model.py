...
class BianjiaoGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_H', type=float, default=2.5, help='weight for H_loss')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.model_names = ['G_low', 'G_high', 'F_low', 'F_high', 'D_low', 'D_high']
        else:  # during test time, only load G
            self.model_names = ['G_low', 'G_high']

        # define networks (both generator and discriminator)
        self.netG_low = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt)
        self.netG_high = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt)
        self.netF_low = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
        self.netF_high = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)

        if self.isTrain:
            self.netD_low = networks.define_D(opt.output_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_low.parameters(), self.netG_high.parameters()), lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_low.parameters(), self.netD_high.parameters()), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def data_dependent_initialize(self, data, iter):
        ...
        if self.opt.isTrain:
            ...
            if self.opt.lambda_NCE > 0.0:
                self.optimizer_F = torch.optim.Adam(itertools.chain(self.netF_low.parameters(), self.netF_high.parameters()), lr=self.opt.lr, betas=(self.opt.beta1, self.opt.beta2))
    ...
    def optimize_parameters(self, iter):
        ...
        self.set_requires_grad([self.netD_low, self.netD_high], True)
        ...
        # update G
        self.set_requires_grad([self.netD_low, self.netD_high], False)
    ...
    def forward(self,iter):
        ...
        self.fake_B_center = self.netG_high(self.real_A_center)
        self.fake_B_resize = self.netG_low(self.real_A_resize)

        self.idt_B_center = self.netG_high(self.real_B_center)
        self.idt_B_resize = self.netG_low(self.real_B_resize)

    def compute_D_high_loss(self):
        """Calculate GAN loss for the discriminator"""
        fake_high = self.fake_B_center.detach()
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake = self.netD_high(fake_high)
        self.loss_D_fake_high = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netD_high(self.real_B_center)
    ...
    def compute_D_low_loss(self):
        ...
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake = self.netD_low(fake_low)
        self.loss_D_fake_low = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netD_low(self.real_B_resize)
        loss_D_real = self.criterionGAN(self.pred_real, True)
    ...
    def compute_G_loss(self, iter):
        ...
        if self.opt.lambda_GAN > 0.0:
            pred_fake_high = self.netD_high(fake_high)
            pred_fake_low = self.netD_low(fake_low)
            self.loss_G_GAN = (self.criterionGAN(pred_fake_high, True).mean() + self.criterionGAN(pred_fake_low, True).mean()) * self.opt.lambda_GAN
        else:
            self.loss_G_GAN = 0.0

        if self.opt.lambda_NCE > 0.0:
            self.loss_NCE = self.calculate_NCE_loss(self.netG_high, self.netF_high, self.real_A_center, self.fake_B_center)+ \
                            self.calculate_NCE_loss(self.netG_low, self.netF_low, self.real_A_resize, self.fake_B_resize)
        else:
            self.loss_NCE, self.loss_NCE_bd = 0.0, 0.0

        if self.opt.lambda_H > 0.0:
            self.loss_H = self.calculate_H_loss(self.real_A_center, self.fake_B_center)+ \
                          self.calculate_H_loss(self.real_A_resize, self.fake_B_resize)
        else:
            self.loss_H = 0.0
        ...
        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            self.loss_NCE_Y = (self.calculate_NCE_loss(self.netG_high, self.netF_high, self.real_B_center, self.idt_B_center) + \
                               self.calculate_NCE_loss(self.netG_low, self.netF_low, self.real_B_resize, self.idt_B_resize))
    ...
    def calculate_NCE_loss(self, netG, netF, src, tgt):
        n_layers = len(self.nce_layers)
        feat_q = netG(tgt, self.nce_layers, encode_only=True)

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]

        feat_k = netG(src, self.nce_layers, encode_only=True)
